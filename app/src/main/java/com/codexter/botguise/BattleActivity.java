...
public class BattleActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<ArrayList<String>> {
        ...
        sendMessage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(mMessage)) {
                    mDatabaseReference.push().setValue(new ChatMessage(mMessage, mUsername, USER));
                }
        ...
        builder.setPositiveButton("Enter", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                if (TextUtils.isEmpty(editText.getText().toString())) {
                    ...
                    GAME_ID = editText.getText().toString().trim();
                    score_opponent = 0;
                    score_user = 0;
                    updateScore();
                    dialog.dismiss();
    ...
    private void initFirebase() {
        ...
        mDatabaseReference = mFirebaseDatabase.getReference().child("Battles").child(GAME_ID);
    ...
    public void onLoadFinished(Loader<ArrayList<String>> loader, ArrayList<String> data) {
        if (loader.getId() == GET_MESSAGE_LOADER_ID) {
            for (int i = 0; i < data.size(); i++) {
                mDatabaseReference.push().setValue(new ChatMessage(data.get(i), mUsername, BOT));
            }
